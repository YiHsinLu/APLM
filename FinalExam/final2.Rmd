...
```{r}
finProd_s1 = finProd[, c(1,2,4,8)]
finProd_s2 = finProd[, c(1,2,3,4,8,9,11,13)]
lm_s1 = lm(Y~X1+X3+X7, data = finProd)
lm_s2 = lm(Y~X1+X2+X3+X7+X8+X10+X12, data = finProd)
```

```{r}
anova_s1 = anova_info(finProd_s1, lm_s1)
anova_s2 = anova_info(finProd_s2, lm_s2)
```

```{r}
y_s1 = (c(1,100,50,50)%*%t(t(lm_s1$coefficients)))[1,1]
y_s2 = (c(1,20,25,50,10,10,50,35)%*%t(t(lm_s2$coefficients)))[1,1]
```